...
この配布物は安定版v2.16.0を固定したもの。ここに書かれた相対パスは、このSKILL.mdの実体ディレクトリから解決する。通常の生成・検証では `ARCHIFY_UPDATE_CHECK_DISABLED=1 node <このスキルの実体パス>/bin/archify.mjs ...` と実行し、更新通知用の通信・状態書き込みを無効にする。下のUpdate awarenessも同じ環境変数で実行し、固定版のまま作業を続ける。

現行コードを説明する場合は既存のObservatoryレポートと該当する実ファイルを根拠にする。書いた関係と実行時に確認した関係を区別する。全体のファイル一覧は `$codebase-atlas`、読み手へ伝える代表フローはこのスキルを使う。対象のAGENTS.mdと既存仕様を尊重し、最小の変更・検証で進める。

公開GitHubの固定リビジョンを持たないローカルコードでは、`component.sources` を使うとrepository-evidence検査が拒否する。公開URLを作り足さず、出典のファイル・行・確認時点を隣接する根拠表（Markdown/JSON）へ記録する。図のデータと根拠表を一緒に渡す。

ブラウザ操作は現在の環境で指定されているブラウザツールを優先する。`visual-check` の実行経路が利用できない場合はdelivery-contractの手動記録に従い、取得した画面・寸法・操作結果と未検証項目を分ける。検証コマンドの成功だけを目視確認済みとしない。日本語のラベルと説明を使い、英語のViewer UIとは区別する。
